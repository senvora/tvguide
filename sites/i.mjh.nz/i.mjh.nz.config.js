  ...
  async channels({ provider }) {
    ...
    for (const option of providerOptions) {
      ...
      data.channels.forEach(item => {
        const name =
          item?.name?.[0]?.value?.trim() ||
          item?.['display-name']?.[0]?.value?.trim() ||
          item.id ||
          'Unknown'

        const icon =
          item?.icon?.[0]?.src || null

        if (!name || name === 'Unknown') {
          console.warn(`[WARN] Missing name for channel id=${item.id} in ${option.path}`)
        }
